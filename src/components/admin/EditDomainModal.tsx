...
export function EditDomainModal({ domain, users, onClose }: EditDomainModalProps) {
  const router = useRouter()
  const [name, setName] = useState(domain.name)
  const [description, setDescription] = useState(domain.description || '')
  const [members, setMembers] = useState<UserOption[]>(domain.members || [])
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <h3 className="text-sm font-semibold text-slate-900 mb-4">Edit Domain</h3>
