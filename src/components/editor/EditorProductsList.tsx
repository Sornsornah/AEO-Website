...
export function EditorProductsList({ initialProducts }: { initialProducts: EditorProduct[] }) {
  ...
  // Ref always holds the current products so the drag closure never goes stale
  ...
  const startDrag = useCallback((e: React.PointerEvent<HTMLButtonElement>, id: string) => {
    ...
    function onUp() {
      ...
      setDraggingId(null)
      // Capture ids immediately — order is the latest local state of the drag
      const ids = order.map((p) => p._id)
      setSaving(true)
  ...
  return (
    <div>
      ...
      {saving && <p className="text-xs text-slate-400 text-right mb-1">Saving order…</p>}
      <div ref={listRef} className="space-y-2">
        {products.map((p) => (
          <div
            key={p._id}
            className={`flex items-center gap-3 p-4 border rounded-xl bg-white select-none transition-opacity duration-100 ${
              draggingId === p._id ? 'opacity-40 border-blue-300' : 'border-slate-200'
            }`}
          >
            <button
